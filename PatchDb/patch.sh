...
export DBID=$(oci db database list --db-system-id=$dbsystemid --compartment-id=ocid1.compartment.oc1..aaaaaaaach4c3i5kpecjfjqwp3wfjfmbbjniyzieaxhcjs6c73ya2wl6vaxq | jq .data[].id)
echo "DBID = $DBID"

export DBPATCHID=$(oci db patch list by-db-system --all --db-system-id=$dbsystemid | jq .data[].id)
